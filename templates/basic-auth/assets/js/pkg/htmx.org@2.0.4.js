var htmx = (function () {
	...
	function querySelectorAllExt(elt, selector, global) {
		...
		while (parts.length > 0) {
			...
			if (selector.indexOf("closest ") === 0) {
				item = closest(
					asElement(elt),
					normalizeSelector(selector.substr(8)),
				);
		...
		if (unprocessedParts.length > 0) {
			...
			result.push(
				...toArray(rootNode.querySelectorAll(standardSelector)),
			);
		}
	...
	function swap(target, content, swapSpec, swapOptions) {
		...
		if (swapSpec.swapStyle === "textContent") {
			...
			forEach(
				...
				/** @param {HTMLTemplateElement} template */ function (
					template,
				) {
					if (
						template.content &&
	...
	function boostElement(elt, nodeData, triggerSpecs) {
		if (
			...
			(elt.tagName === "FORM" &&
				String(getRawAttribute(elt, "method")).toLowerCase() !==
					"dialog")
		) {
			...
			const eventListener = function (evt) {
				...
				if (eventData.handledFor.indexOf(elt) < 0) {
					...
					if (triggerSpec.changed) {
						...
						const lastValue =
							elementData.lastValue.get(triggerSpec);
						if (
							lastValue.has(node) &&
							lastValue.get(node) === value
						) {
							return;
						}
					...
					if (triggerSpec.throttle > 0) {
						if (!elementData.throttle) {
							...
							handler(elt, evt);
							elementData.throttle = getWindow().setTimeout(
								function () {
	...
	function initNode(elt) {
		...
		if (nodeData.initHash !== attrHash) {
			...
			const hasExplicitHttpAction = processVerbs(
				elt,
				nodeData,
				triggerSpecs,
			);
			...
			if (split.length > 0) {
				for (let i = 0; i < split.length; i++) {
					...
					if (value.indexOf("swap:") === 0) {
						...
						const focusScrollVal = value.slice(
							"focus-scroll:".length,
						);
	...
	function handleAjaxResponse(elt, responseInfo) {
		...
		let ignoreTitle =
			htmx.config.ignoreTitle || responseHandling.ignoreTitle;
